...
class Simple_Job_Board_Shortcode_Jobpost {
    ...
    public function jobpost_shortcode( $atts, $content ) {
        ...
        $atts = is_array( $atts ) ? array_map( 'sanitize_text_field', $atts ) : '';
        ...
        $args = apply_filters(
            'sjb_output_jobs_args', array(
                'post_status'  => 'publish', 
                'posts_per_page' => $shortcode_args['posts'],
                'post_type' => 'jobpost',
                'paged' => $paged,
            ), $atts
        );
        ...
        $args['jobpost_category'] = ( NULL != filter_input( INPUT_GET, 'selected_category' )  && -1 != filter_input(INPUT_GET, 'selected_category') ) ? filter_input(INPUT_GET, 'selected_category'): esc_attr( $shortcode_args['category'] );
        $args['jobpost_job_type'] = ( NULL != filter_input( INPUT_GET, 'selected_jobtype' ) && -1 != filter_input(INPUT_GET, 'selected_jobtype') ) ? filter_input(INPUT_GET, 'selected_jobtype') : esc_attr( $shortcode_args['type'] );
        $args['jobpost_location'] = ( NULL != filter_input( INPUT_GET, 'selected_location' ) && -1 != filter_input(INPUT_GET, 'selected_location') ) ? filter_input(INPUT_GET, 'selected_location') : esc_attr( $shortcode_args['location'] );       
